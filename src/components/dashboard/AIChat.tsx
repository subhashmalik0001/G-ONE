...
export default function AIChat() {
    ...
    const [messages, setMessages] = useState<{ role: 'user' | 'assistant', content: string }[]>([
        { role: 'assistant', content: "Hello! I'm your G-ONE Clinical AI Health Assistant. You can ask me anything about your symptoms, medical records, lab results, or vitals." }
    ]);
    ...
    const handleSend = async () => {
        ...
        try {
            const advice = await aiClient.getHealthAdvice(userMsg);
            const responseText = advice.advice || "Based on clinical guidelines, please monitor your symptoms and consult a healthcare professional for specific diagnosis.";
            setMessages(prev => [
                ...prev,
            ...
            setMessages(prev => [
                ...prev,
                { role: 'assistant', content: "Your vitals show stable heart rate and normal blood pressure readings. Please maintain your hydration, balanced nutrition, and regular exercise." }
            ]);
        } finally {
            setIsLoading(false);
        }
        ...
        <div className={cn(
            ...
            {!isOpen ? (
                <button 
                    ...
                    title="Open G-ONE Clinical AI Assistant"
                >
                ...
                <div className={cn(
                    ...
                    isMaximized ? "fixed inset-6 w-auto h-auto z-[60]" : "w-[400px] h-[600px] max-h-[80vh]"
                )}>
                    {/* Header */}
                    <div className="p-6 bg-[#05050a] text-white flex items-center justify-between">
                        <div className="flex items-center gap-3">
                            ...
                            <div>
                                <h3 className="text-[15px] font-black tracking-tight" style={{ fontFamily: "var(--font-display)" }}>
                                    G-ONE AI Assistant
                                </h3>
                        ...
                        <div className="flex items-center gap-2">
                            <button 
                                ...
                                className="h-8 w-8 rounded-lg hover:bg-white/10 flex items-center justify-center transition-colors text-white"
                    ...
                    {/* Messages Container */}
                    <div ref={scrollRef} className="flex-1 overflow-y-auto p-6 space-y-6 scroll-smooth bg-[#fafaf8]/50">
                        {messages.map((m, i) => (
                        ...
                        {isLoading && (
                            <div className="flex items-center gap-3">
                                ...
                                <div className="bg-white border border-black/5 p-4 rounded-[20px] rounded-tl-none flex items-center gap-1.5">
                                    <span className="w-1.5 h-1.5 bg-[#b8ff00] rounded-full animate-bounce [animation-delay:-0.3s]" />
                                    <span className="w-1.5 h-1.5 bg-[#b8ff00] rounded-full animate-bounce [animation-delay:-0.15s]" />
                                    <span className="w-1.5 h-1.5 bg-[#b8ff00] rounded-full animate-bounce" />
                                </div>
                            </div>
                        )}
                    </div>
                    ...
                    <div className="p-4 sm:p-6 border-t border-black/5 bg-white">
                        <div className="relative flex items-center gap-3 bg-[#fafaf8] border border-black/5 rounded-2xl p-2 px-4 focus-within:border-[#05050a] focus-within:ring-1 focus-within:ring-[#05050a] transition-all">
                            <input 
                                type="text"
                                placeholder="Ask about your health or symptoms..."
                                className="flex-1 bg-transparent border-none outline-none text-[13px] font-bold py-2 text-[#05050a]"
                                value={input}
                        ...
                        <p className="text-[10px] text-center mt-3 font-black uppercase tracking-[0.2em] text-[#b0b0b0]">
                            Powered by G-ONE Clinical AI
                        </p>
                    </div>
                </div>
            )}
